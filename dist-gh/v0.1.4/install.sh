...
main() {
  ...
  elif [[ -n "${LAP_PAIR_API_URL:-}" ]]; then
    # A public-release install needs only the pair endpoint.  Before pairing,
    # that endpoint is also a safe asset-proxy fallback; after pairing, the
    # daemon persists and prefers asset_base_url returned by POST /v1/pair.
    default_saas_url="${LAP_PAIR_API_URL%/}"
